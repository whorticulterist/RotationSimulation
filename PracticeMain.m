function PracticeMain(textureon, dotson, normalson, stereomode)

% create random list for ellip side placement
% default is that E is on left(0) and right is 1
ellipSide = randi(1000,100);

for i = 1:length(ellipSide)
    ellipSide(i) = mod(ellipSide(i),2);
end
...
objectOrder = objectOrder(randperm(length(objectOrder)));
% trial counter(up to 100)
CountObj = 1;
% load in obj in a String list
Objects = dir("objects");
% remove extra junk: '.DS_Store', '..', '.'
%Objects(Objects == '.') = [];
%Objects(Objects == '..') = [];
%Objects(Objects == '.DS_Store') = [];
...
% 'g' and 'h' == Rotate object around z axis.
% 't' and 'y' == rotate object around x axis.
% 'v' and 'b' == rotate object around y axis.
% 'space' == select ellip
% '1' == view object
% '2' == view first ellip
% '3' == view second ellip
% 'ESC' == Quit demo.
%
% note: these rotations are commutative aka improper rotations
%       in other words this is an affine transformation composed
%       of a rotation about an axis and a reflection of the plane
...
rotateleft = KbName('g');
rotateright = KbName('h');
rotateup = KbName('y');
rotatedown = KbName('t');
rotateclock = KbName('b');
rotatecounterclock = KbName('v');
selectRight = KbName('j');
selectLeft = KbName('f');
...
% for single obj load two versions of the same file
basepath = [fileparts(which(mfilename)) '/'];
objs = [ LoadOBJFile([basepath 'untitled.obj'],0,1) LoadOBJFile([basepath 'untitled.obj'],0,1) ];

% TODO add code for loading in all objs in a double array
...
ang = 0.0;      % Initial rotation angle of left Right

vertang = 0.0;  % initial rotation angle of up down

clockang = 0.0; % initial rotation angle of clock counterclock
...
while ((GetSecs - t) < 60)
    ...
    drawShape(ang, theta, rotatev, dotson, normalson, vertang,clockang);
    ...
    if KeyIsDown
        ...
        if ( KeyIsDown==1 && KeyCode(rotateclock)==1 )
            clockang=clockang+1.0;
            KeyIsDown=0;
        end

        if ( KeyIsDown==1 && KeyCode(rotatecounterclock)==1 )
            clockang=clockang-1.0;
            KeyIsDown=0;
        end

        if ( KeyIsDown==1 && KeyCode(selectLeft)==1 )
            %TODO add code for replaceing mesh with next mesh
...
function drawShape(ang, theta, rotatev, dotson, normalson,vertang,clockang)
...
% Setup rotation around axis for left right:
glRotatef(theta,rotatev(1),rotatev(2),rotatev(3));
glRotatef(ang,0,1,0);

% Setup rotation around axis for up down:
glRotatef(theta,rotatev(1),rotatev(2),rotatev(3));
glRotatef(vertang,0,0,1);

% Setup rotation around axis for up down:
glRotatef(theta,rotatev(1),rotatev(2),rotatev(3));
glRotatef(clockang,1,0,0);
